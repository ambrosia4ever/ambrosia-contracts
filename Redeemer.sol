...
contract AmbrosiaRedeemer is Ownable {
    ...
    function redeem(uint _amount) external{
        ...
        for (uint i=0;i<deemToke.length;i++) {
            ...
            uint xamount = portion.mul(balance).mul(1000000-tax).div(1000000000).div(1000000);
    ...
    function addRedeemToken(address _token, uint _tax, uint _daotax) external onlyPolicy() {
    ...
    function adjustTax(uint _index, address _token, uint _tax, uint _daotax) external onlyPolicy(){
    	require( _token == deemToke[ _index ].token);
    	deemToke[ _index ].tax = _tax;
    	deemToke[ _index ].daotax = _daotax;
    }
